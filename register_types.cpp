...
static Vector<FlowScriptNodeTypeInfo> _GET_BUILT_IN_FLOW_SCRIPT_TYPE_LIST()
{
	return Vector<FlowScriptNodeTypeInfo> {
		FlowScriptNodeTypeInfo {
			.enabled = true,
			.type_name = "Procedure",
			.type_category = "Built-In",
...
void initialize_flow_script_module(ModuleInitializationLevel p_level)
{
	...
	if (p_level == MODULE_INITIALIZATION_LEVEL_EDITOR)
	{
		...
		// }
		for (const FlowScriptNodeTypeInfo &curr_type : _GET_BUILT_IN_FLOW_SCRIPT_TYPE_LIST())
		{
			FlowScriptNodeTypeDB::get_singleton()->add_type(curr_type);
		}

...
static func create_settings() -> void:
	for setting in SETTINGS_LIST:
		...
		if "require_restart" in setting:
			ProjectSettings.set_restart_if_changed(setting.name, setting.require_restart)
		if "is_basic" in setting:
			ProjectSettings.set_as_basic(setting.name, setting.is_basic)
